...
abstract class ChatViewModel : ViewModel() {
  ...
  /**
   * Entry point for sending a message. Handles adding the message to the UI and setting the loading
   * state.
   */
  ...
  protected fun validateAndDisplayResponse(
    ...
    if (
      ...
      messages.value =
        currentMessages + UiChatMessage(candidate.content, candidate.groundingMetadata)
      attachments.value = emptyList()
      uiState.value = ChatUiState.Success
    }
  }

  fun attachFile(fileInBytes: ByteArray, mimeType: String?, fileName: String? = "Unnamed file") {
