...
@Serializable object WeatherChatRoute
...
class WeatherChatViewModel : ChatViewModel() {
  ...
  private suspend fun handleFunctionCalls(
    ...
    response.functionCalls.forEach { functionCall ->
      Log.d("WeatherChatViewModel", "Model responded with function call:" + functionCall.name)
        ...
        "fetchWeather" -> {
          ...
          val finalResponse =
            if (city == null || state == null || date == null) {
                    ...
                    FunctionResponsePart(
                      ...
                      JsonObject(
                        mapOf(
                          "error" to
                            JsonPrimitive(
                              "Unable to fetch weather - one of the parameters was null"
                            )
                        )
                      ),
                    )
              ...
              val functionResponse = WeatherRepository.fetchWeather(city, state, date)
