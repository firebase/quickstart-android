...
@Serializable object ServerPromptTemplateRoute
...
class ServerPromptTemplateViewModel : ViewModel() {
  ...
  init {
    templateGenerativeModel =
      Firebase.ai(backend = GenerativeBackend.googleAI()).templateGenerativeModel()
  }
      ...
      try {
        val response =
          templateGenerativeModel.generateContent(
            ...
            mapOf("customerName" to inputText),
          )
        uiState.value = ServerPromptUiState.Success(response.text)
      } catch (e: Exception) {
        uiState.value =
          ServerPromptUiState.Error(
