...
@Serializable object HybridInferenceRoute
    ...
    field =
      MutableStateFlow(
        HybridInferenceUiState(
          expenses =
            listOf(Expense("Lunch", 15.50, "Example data"), Expense("Coffee", 4.75, "Example data"))
        )
      )
          ...
          model.onDeviceExtension?.download()?.collect { downloadStatus ->
            when (downloadStatus) {
              ...
              is DownloadStatus.DownloadFailed -> {
                uiState.update {
                  it.copy(modelStatus = "Download failed", errorMessage = "Model download failed")
                }
              }
            }
          }
