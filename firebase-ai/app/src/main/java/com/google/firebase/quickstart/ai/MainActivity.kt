        ...
        Scaffold(
          ...
          NavHost(
            ...
            FIREBASE_AI_SAMPLES.forEach { sample ->
              composable(route = sample.route::class, typeMap = emptyMap()) {
                val viewModelClass = sample.viewModelClass?.java ?: return@composable
                ...
                when (sample.screenType) {
                  ...
                  ScreenType.BIDI -> {
                    (vm as? BidiViewModel)?.let {
                      @SuppressLint("MissingPermission") StreamRealtimeScreen(it)
                    }
                  }

                  ScreenType.BIDI_VIDEO -> {
                    (vm as? BidiViewModel)?.let {
                      @SuppressLint("MissingPermission") StreamRealtimeVideoScreen(it)
                    }
                  }
