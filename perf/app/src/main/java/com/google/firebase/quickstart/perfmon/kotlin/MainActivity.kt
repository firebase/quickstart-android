    ...
    Thread(
        Runnable {
          try {
            ...
            trace.stop()
            runOnUiThread { Toast.makeText(this, "Trace completed", Toast.LENGTH_SHORT).show() }
          }
        }
      )
      .start()
    ...
    lifecycleScope.launch {
      withContext(Dispatchers.IO) {
        ...
        if (contentFile.createNewFile()) {
          // Content file exist did not exist in internal storage and new file was
          // created.
      ...
      .addOnCompleteListener(
        OnCompleteListener { task ->
          if (!task.isSuccessful) {
            ...
            Toast.makeText(this, getString(R.string.text_read_error), Toast.LENGTH_LONG).show()
          ...
          trace.incrementMetric(FILE_SIZE_COUNTER_NAME, fileContent!!.toByteArray().size.toLong())
