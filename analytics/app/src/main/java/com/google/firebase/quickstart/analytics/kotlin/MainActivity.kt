...
class MainActivity : AppCompatActivity() {
  ...
  /**
   * The [androidx.viewpager2.widget.PagerAdapter] that will provide fragments for each image. This
   * uses a [FragmentStateAdapter], which keeps every loaded fragment in memory.
   */
  ...
  /**
   * Display a dialog prompting the user to pick a favorite food from a list, then record the
   * answer.
   */
  ...
  private fun getUserFavoriteFood(): String? {
    return PreferenceManager.getDefaultSharedPreferences(this).getString(KEY_FAVORITE_FOOD, null)
  }
  ...
  /**
   * Record a screen view for the visible [ImageFragment] displayed inside [FragmentStateAdapter].
   */
  ...
  /**
   * This sample has a single Activity, so we need to manually record "screen views" as we change
   * fragments.
   */
