  ...
  private fun fetchMovie() {
    viewModelScope.launch {
      try {
        ...
        val movie = moviesConnector.getMovieById.execute(id = UUID.fromString(movieId)).data.movie
        ...
        _uiState.value =
          if (user == null) {
            ...
            val isFavorite =
              moviesConnector.getIfFavoritedMovie
                .execute(movieId = UUID.fromString(movieId))
                .data
                .favorite_movie != null
    ...
    viewModelScope.launch {
      try {
        if (newValue) {
          ...
          moviesConnector.deleteFavoritedMovie.execute(movieId = UUID.fromString(movieId))
        }
