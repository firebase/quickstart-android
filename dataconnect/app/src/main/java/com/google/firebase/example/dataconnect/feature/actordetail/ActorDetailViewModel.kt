  ...
  private fun fetchActor() {
    viewModelScope.launch {
      try {
        ...
        val actor = moviesConnector.getActorById.execute(id = UUID.fromString(actorId)).data.actor

        _uiState.value = ActorDetailUIState.Success(actor = actor, isUserSignedIn = user != null)
      } catch (e: Exception) {
