      ...
      is MovieDetailUIState.Success -> {
        ...
        Column(modifier = Modifier.verticalScroll(scrollState)) {
          MovieInformation(
            ...
            onFavoriteToggled = { newValue -> movieDetailViewModel.toggleFavorite(newValue) },
          )
          // Main Actors list
          ActorsList(
            listTitle = stringResource(R.string.title_main_actors),
            actors =
              movie?.mainActors?.mapNotNull { Actor(it.id.toString(), it.name, it.imageUrl) },
          ...
          ActorsList(
            ...
            actors =
              movie?.supportingActors?.mapNotNull { Actor(it.id.toString(), it.name, it.imageUrl) },
            onActorClicked = { onActorClicked(it) },
          )
          UserReviews(
            onReviewSubmitted = { rating, text -> movieDetailViewModel.addRating(rating, text) },
            movie?.reviews,
          )
        }
      }
    ...
    Column(modifier = modifier.padding(16.dp)) {
      ...
      Row {
        AsyncImage(
          ...
          modifier = Modifier.width(150.dp).aspectRatio(9f / 16f).padding(vertical = 8.dp),
        )
