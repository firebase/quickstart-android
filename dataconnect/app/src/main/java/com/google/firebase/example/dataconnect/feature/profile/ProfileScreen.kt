  ...
  when (uiState) {
    ...
    is ProfileUIState.AuthState -> {
      AuthScreen(
        ...
        onSignIn = { email, password -> profileViewModel.signIn(email, password) },
      )
    }
...
fun ProfileScreen(
  ...
  Column(modifier = Modifier.padding(vertical = 16.dp).verticalScroll(scrollState)) {
    ...
    TextButton(onClick = { onSignOut() }, modifier = Modifier.padding(horizontal = 16.dp)) {
      Text("Sign out")
    }
    ...
    MoviesList(
      ...
      movies =
        favoriteMovies.mapNotNull {
          Movie(
            it.movie.id.toString(),
            it.movie.imageUrl,
            it.movie.title,
            it.movie.rating?.toFloat(),
          )
        },
      onMovieClicked = onMovieClicked,
    )
...
@Composable
fun ReviewsList(userName: String, reviews: List<GetCurrentUserQuery.Data.User.ReviewsItem>) {
  Column {
