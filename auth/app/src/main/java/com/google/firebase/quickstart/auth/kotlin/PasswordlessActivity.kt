...
class PasswordlessActivity : BaseActivity(), View.OnClickListener {
  ...
  /**
   * Check to see if the Intent has an email link, and if so set up the UI accordingly. This can be
   * called from either onCreate or onNewIntent, depending on how the Activity was launched.
   */
  ...
  /**
   * Sign in using an email address and a link, the link is passed to the Activity from the dynamic
  ...
  private fun updateUI(user: FirebaseUser?) {
    if (user != null) {
      binding.status.text =
        getString(R.string.passwordless_status_fmt, user.email, user.isEmailVerified)
