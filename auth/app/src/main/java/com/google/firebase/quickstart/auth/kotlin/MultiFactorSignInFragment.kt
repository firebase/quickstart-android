...
class MultiFactorSignInFragment : BaseFragment() {
  ...
  private fun generateFactorOnClickListener(
  ...
  private fun onClickFinishSignIn() {
    if (lastPhoneAuthCredential == null) {
      if (TextUtils.isEmpty(binding.smsCode.text.toString())) {
        Toast.makeText(context, "You need to enter an SMS code.", Toast.LENGTH_SHORT).show()
        return
      }
      lastPhoneAuthCredential =
        PhoneAuthProvider.getCredential(lastVerificationId!!, binding.smsCode.text.toString())
    }
    multiFactorResolver
      .resolveSignIn(PhoneMultiFactorGenerator.getAssertion(lastPhoneAuthCredential!!))
      .addOnSuccessListener { findNavController().navigate(R.id.action_mfasignin_to_mfa) }
