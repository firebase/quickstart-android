...
class MyFirebaseMessagingService : FirebaseMessagingService() {
   ...
   *
   * This method provides the unique Firebase Installation ID (FID), which should be used to target
   * this app instance for direct-send messaging.
   *
   ...
   *
   * Ensure the provided `installationId` is uploaded if it hasn't been previously or it might have
   * been deleted on 404s.
   *
   * **Note:** To use this API, you must enable it by adding `<meta-data
  ...
  public override fun onUnregistered(installationId: String) {
    ...
    // so that the app server will not try to send FCM messages to the un registered app
    // instance.
   ...
   *
   * Modify this method to associate the user's FCM registration with any server-side account
   ...
   *
   * Modify this method to disassociate the user's FCM registration with any server-side account
   * maintained by your application.
   *
